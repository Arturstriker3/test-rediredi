...
describe("Categories POST", () => {
  it("should call post method with correct URL and data", async () => {
    ...
    expect(axiosInstance.post).toHaveBeenCalledWith(`${urlBase}`, { name });
    expect(response).toEqual(mockResponse);
  });
});
...
describe("Categories POST Children", () => {
  it("should call postChildren method with correct URL and data", async () => {
    ...
    const mockResponse = {
      data: { id: "child-123", name, parent: { id: parentId } },
    };
    ...
    expect(axiosInstance.post).toHaveBeenCalledWith(`${urlBase}`, {
      parent: { id: parentId },
      name: name,
    });
    expect(response).toEqual(mockResponse);
